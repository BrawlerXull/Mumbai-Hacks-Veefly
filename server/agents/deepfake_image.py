...
class DeepfakeImagePredictor:
    """
    A class to predict deepfake probability for a single image using Hugging Face model.
    ...
    def predict(self, image_path):
        ...
        Returns:
            float: Probability of being real (0 to 1).
        """
        self.load_model()
        ...
        try:
            predictions = self.pipeline(image_path)
            ...
            # If we found explicit scores, use them. 
            # If the model uses different labels, we might need to adjust.
            # Based on model card, labels are likely 'Real' and 'Fake'.
